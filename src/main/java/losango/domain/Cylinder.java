...
public class    Cylinder {
    ...
    public Hexagon getHexagonFromPoint(double x, double y) {
        double q = (x * Math.sqrt(3.0)/3.0 - y/3.0) / SIZE;
        double r = y * 2.0/3.0 / SIZE;
    ...
    public Point fromLatLngToMercatorPoint(double latitude, double longitude) {
        ...
        double y = ((1 - Math.log(Math.tan(phi) + 1 / Math.cos(phi)) / Math.PI) / 2.0) * TILE_SIZE;
        return new Point(x,y);
    }

    public Point getCentralCoordinates(Hexagon hexagon) {
        double x = SIZE * Math.sqrt(3) * (hexagon.getColumn() + hexagon.getRow()/2.0);
        double y = SIZE * 3.0/2 * hexagon.getRow();
        return new Point(x,y);
    }
    ...
    public Point[] getHexagonCorners(Hexagon hexagon) {
        ...
        return IntStream.range(0, 6).mapToObj(i -> {
            ...
            return new Point(centre.getX() + SIZE * Math.cos(angle),
                    centre.getY() + SIZE * Math.sin(angle));
        }).toArray(Point[]::new);
    }
